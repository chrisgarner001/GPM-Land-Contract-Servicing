...
import { getLenderOptions, getPrincipalChangeData } from "@/server/lenderReports";
import { formatCents, formatDate } from "@/lib/format";
import { emailPrincipalChangeAction, postPrincipalChangeAction } from "./actions";
...
export default async function PortfolioChangeInPrincipalPage({
  searchParams,
}: {
  ...
  const statements = selectedIds.length > 0 ? await getPrincipalChangeData(selectedIds, startDate, endDate) : [];
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-8">
      ...
      <h1 className="mt-2 mb-1 text-xl font-semibold text-slate-900 print:hidden">Portfolio Change in Principal</h1>
      <p className="mb-4 text-sm text-slate-500 print:hidden">Principal balance change across a lender's portfolio over a date range.</p>
        ...
        <div className="space-y-8">
          {statements.map((data) => (
            <section key={data.lenderId} className="rounded-lg border border-slate-200 bg-white shadow-sm">
              <div className="p-4">
                <ReportActionBar
                  excelHref={`/reports/lender/portfolio-change-in-principal/export?lenderIds=${data.lenderId}&startDate=${startDate}&endDate=${endDate}`}
                  onEmail={emailPrincipalChangeAction.bind(null, data.lenderId, startDate, endDate)}
                  onPost={postPrincipalChangeAction.bind(null, data.lenderId, startDate, endDate)}
                  postLabel="Post to Lender Portal"
                />

                <h2 className="text-lg font-semibold text-slate-900">
                  Portfolio Change in Principal — {lendersById.get(data.lenderId)?.displayName ?? "Unknown Lender"}
                </h2>
                  ...
                  <table className="w-full min-w-[400px] text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                        ...
                        <th className="px-3 py-2 text-right">Principal Change</th>
                      </tr>
                    </thead>
                      ...
                      {data.rows.length === 0 ? (
                        <tr>
                          <td colSpan={2} className="px-3 py-4 text-center text-slate-400">
                            No principal credited in this range.
                          </td>
                        </tr>
                      ) : (
                        data.rows.map((r) => (
                          <tr key={r.contractId}>
                            ...
                            <td className="px-3 py-2 text-right">{formatCents(r.principalChangeCents)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                    <tfoot>
                      <tr className="border-t border-slate-200 font-medium">
                        ...
                        <td className="px-3 py-2 text-right">{formatCents(data.totalPrincipalChangeCents)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
