...
import { getLenderOptions, getAccruedInterestData } from "@/server/lenderReports";
import { formatCents, formatDate, formatPercent } from "@/lib/format";
import { emailAccruedInterestAction, postAccruedInterestAction } from "./actions";
...
export default async function AccruedInterestPage({
  searchParams,
}: {
  ...
  const statements = selectedIds.length > 0 ? await getAccruedInterestData(selectedIds, startDate, endDate) : [];
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      ...
      <h1 className="mt-2 mb-1 text-xl font-semibold text-slate-900 print:hidden">Accrued Interest</h1>
      <p className="mb-4 text-sm text-slate-500 print:hidden">Interest accrued but not yet distributed, per lender, over a date range.</p>
        ...
        <div className="space-y-8">
          {statements.map((data) => (
            <section key={data.lenderId} className="rounded-lg border border-slate-200 bg-white shadow-sm">
              <div className="p-4">
                <ReportActionBar
                  excelHref={`/reports/lender/accrued-interest/export?lenderIds=${data.lenderId}&startDate=${startDate}&endDate=${endDate}`}
                  onEmail={emailAccruedInterestAction.bind(null, data.lenderId, startDate, endDate)}
                  onPost={postAccruedInterestAction.bind(null, data.lenderId, startDate, endDate)}
                  postLabel="Post to Lender Portal"
                />

                <h2 className="text-lg font-semibold text-slate-900">Accrued Interest — {lendersById.get(data.lenderId)?.displayName ?? "Unknown Lender"}</h2>
                  ...
                  <table className="w-full min-w-[800px] text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {data.holdings.length === 0 ? (
                        ...
                        data.holdings.map((h) => (
                          <tr key={h.contractId}>
                            <td className="px-3 py-2">{h.contractNumber}</td>
                            <td className="px-3 py-2">{h.borrowerName}</td>
                            <td className="px-3 py-2 text-right">{formatPercent(h.ownershipPercent)}</td>
                            <td className="px-3 py-2 text-right">{formatPercent(h.interestRateAnnual)}</td>
                            <td className="px-3 py-2 text-right">{formatCents(h.principalBalanceCents)}</td>
                            <td className="px-3 py-2 text-right">{h.days}</td>
                            <td className="px-3 py-2 text-right">{formatCents(h.accruedInterestCents)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
                    <tfoot>
                      <tr className="border-t border-slate-200 font-medium">
                        ...
                        <td className="px-3 py-2 text-right">{formatCents(data.totalAccruedInterestCents)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
                <p className="mt-3 text-xs text-slate-400">
                  Interest accrued since the last serviced due date but not yet distributed — a projection as of {formatDate(endDate)}, not a
                  stored/paid figure.
                </p>
              </div>
            </section>
