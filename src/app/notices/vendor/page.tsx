...
export default async function VendorNoticesPage() {
  const templates = await db
    .select()
    .from(noticeTemplates)
    .where(eq(noticeTemplates.category, "VENDOR"))
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-8">
      ...
      <p className="mb-4 text-sm text-slate-500">Vendor notices.</p>
      ...
      {templates.length === 0 ? (
        <p className="text-sm text-slate-400">No vendor templates yet — build one in Template Builder.</p>
      ) : (
